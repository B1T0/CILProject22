...
class model(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        phi_IC_negative = self.forward(y_negative)
    ...
    def loss(self, phi_user, phi_item,  phis_IC, phis_UC, phi_IC_negative,
        ...
        for i in range(len(phi_IC_negative)):
            loss_NS += torch.mean(torch.mean(F.logsigmoid(torch.sum(torch.mul(phi_item, phi_IC_negative[i])), dim=1)))
            loss_NS += torch.mean(torch.mean(F.logsigmoid(torch.sum(torch.mul(phi_user, phi_UC_negative[i])), dim=1)))
